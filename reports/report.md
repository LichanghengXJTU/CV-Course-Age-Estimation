...
| 随机种子 | 42（所有运行一致） |
| Git revision | DenseNet 50ep & ViT baseline 50ep：`b13e839`；ViT new 50ep：`605c112`；三个 100ep ablation 运行：`d813d2b`（新增产物在 `23e1ca9` 中归档） |
...
### 11.1 budget × recipe 8 行 ablation 表

数据来自 `results/comparison/budget_ablation.csv`（gt_std = 17.67）：

| 模型 | budget | best epoch | test MAE | test RMSE | pred_std | Pearson r |
|---|:---:|---:|---:|---:|---:|---:|
| DenseNet121 | 25 ep | 22 | 5.064 | 7.190 | 15.29 | 0.920 |
| DenseNet121 | **50 ep** (canonical) | 31 | **4.941** | **6.943** | 15.26 | 0.925 |
| DenseNet121 | 100 ep | 59 | 5.003 | 7.103 | 15.27 | 0.922 |
| ViT-B/16 (new recipe) | **50 ep** (canonical) | 37 | **4.861** | **6.943** | 15.27 | 0.924 |
| ViT-B/16 (new recipe) | 100 ep | 34 | 4.949 | 7.180 | 15.71 | 0.923 |
| ViT-B/16 (baseline recipe) | 25 ep | 25 | 13.009 | 17.627 |  3.09 | 0.286 ← 塌缩 |
| ViT-B/16 (baseline recipe) | **50 ep** (canonical) | 45 |  9.442 | 13.134 | 11.54 | 0.696 |
| ViT-B/16 (baseline recipe) | 100 ep | 65 | 11.957 | 16.299 |  8.51 | 0.429 ← 部分回退 |

加粗为该模型在三档 budget 中的最优。**关键结构性观察**：三个模型 100 ep 都
比 50 ep 退化 —— DN +0.062 MAE、ViT (new) +0.088 MAE、ViT (baseline)
+2.515 MAE。**50 ep 是三个模型共同的 budget sweet spot**。详细分析见 §11.8。
...
4.6 MAE），早期版本「recipe 救了 ViT」的单因素叙事并不完整。
**注意**：等式 `(−3.567) + (−4.581) = −8.148 = Δ_total` 只是沿 (baseline,25)
→ (baseline,50) → (new,50) 两条边走完恒等于直接 (baseline,25) → (new,50)
的路径加法 —— 它本身**不**构成"两个维度可加、无交互项"的实证。
要做正式的可加性 / 交互项检验，需要补出第 4 个 cell `(new, 25ep)` 把 2×2
网格画完整，本实验未做此项（见 §12.2 limitation）。
...
**第三个观察（来自 §11.8 的 100 ep 扩展）**：上述 −3.567 / −4.581 的分解
**只在 25→50 这一段成立**。继续把 budget 从 50 推到 100 ep 后，三个模型的
test MAE **全部回升**（DN 4.941→5.003、ViT new 4.861→4.949、ViT baseline
9.442→11.957）。即 **budget 维度并非单调** ——「更多 epoch = 更好」的
朴素直觉在本实验所有三个模型上都被直接证伪。50 ep 是三个模型共同的
budget sweet spot；进一步的因果机理与 ViT baseline 100 ep 部分回退向
collapse 的证据见 §11.8。
...
### 11.8 100 epoch 扩展实验：non-monotonic budget

为检验「更多 epoch 是否进一步降低 MAE」，三个模型都被加跑了一档 100 epoch
ablation（commit `d813d2b`）。protocol 上唯一改动是把 `epochs` 从 50 设到 100，
**cosine schedule 的 `t_max` 同步从 50 推到 100**；warmup（ViT new 仍为 5 ep）
与 StepLR（DN 仍为 `step_size=10`，γ=0.5）保持不变。三个 100ep 运行的
test 结果与 50 / 25 ep 对照：

| 模型 | 25 ep | 50 ep (canonical) | 100 ep | Δ (50 → 100) |
|---|---:|---:|---:|---:|
| DenseNet121 | 5.064 | **4.941** | 5.003 | **+0.062** |
| ViT-B/16 (new recipe) | — | **4.861** | 4.949 | **+0.088** |
| ViT-B/16 (baseline recipe) | 13.009 | **9.442** | 11.957 | **+2.515** |
...
1. **DenseNet 25→50→100：5.064 → 4.941 → 5.003**。25→50 之间还有
   −0.123 MAE 的边际收益，50→100 反转为 +0.062 —— 进一步确认 §11.2 的元
   洞察：DN 在 25 ep 就已接近自身饱和，多给 epoch 只会带来 late-stage
   low-lr drift 与偶然换 best epoch 的随机性（best epoch 由 31 推到 59，
   但对应 test MAE 反而轻微变差），不再带来真实改进。
2. **ViT (new) 50→100：4.861 → 4.949（+0.088）**。pred_std 从 15.27 微升到
   15.71，Pearson r 几乎不变（0.924→0.923）。这是典型的轻度过拟合：模型
   在 50 ep 时已经处于 val MAE 平台，多 50 ep 的更新只是在训练集上继续抠
   细节，未带来泛化收益。
3. **ViT (baseline) 25→50→100：13.009 → 9.442 → 11.957（U 形）**。**这是
   本节最关键的发现**：baseline 在 100 ep 不仅没继续改进，反而**部分回退向
   collapse 状态** —— pred_std 从 50 ep 的 11.54 退到 8.51，Pearson r 从
   0.696 退到 0.429，per-age 桶上 [70, inf) MAE 从 50 ep 的 34.21 回升到
   41.25、[60, 70) 从 20.76 回升到 25.94。50 ep 时辛苦逃出来的「mean
   predictor 局部最优」在 100 ep 又被部分拉回去。换言之，**朴素的
   "baseline ViT 给更多时间就能 work" 直觉被本实验直接证伪**。

**机理（为什么三个模型都退化）**：本次 ablation 把 `t_max` 与 `epochs` **同步**
拉到 100，所以 cosine schedule 的衰减斜率被整体放缓 —— 前 50 ep 都处于
比 canonical 配方更高的 lr 上：

- ViT (new) 在 epoch 50 处的 cosine 输出从 canonical 的 η_min=3e-7（near zero）
  变成 100ep schedule 下的 lr ≈ 1e-4 量级（cosine 还只走到一半）；
- ViT (baseline) 同理，50ep 时 lr 仍处 ~2.5e-5 量级，而非 canonical 的
  zero；DN 的 StepLR 不受 `t_max` 影响（仍每 10 ep 衰减），但模型本身
  早已饱和，多出的低 lr update 只会引入 drift。

在 ViT 两个 run 上，这导致中段（epoch 30-60）**高 lr 区间停留更久**，模型
持续累积梯度噪声并轻度过拟合，进入 cosine 后段 anneal 时反而是从一个更差
的中间点出发 —— 后半段 anneal 来不及把这部分噪声抹掉。**ViT (baseline)
对此尤其敏感**：它本身就在 50 ep 时只勉强从塌缩状态恢复，中段持续高 lr +
wd=0.05 持续把 head 压向零，再次把已经学到的「年龄区分性」表征侵蚀掉
（pred_std 从 11.54 退到 8.51 是直接证据），50→100 ep 在 baseline 上是
**有害 budget**。

**结论**：budget 与 schedule shape 不能孤立讨论。把 `t_max` 与 `epochs`
绑定（本实验的做法）等价于"把 cosine 整段拉长"，这对已经接近饱和的配方
（DN 50ep、ViT new 50ep）只带来 late-stage drift，对未饱和的差配方
（ViT baseline）反而会在中段引入持续高 lr 噪声，把 50 ep 的部分恢复又拉
回去。**正确做法**：要么保持 `t_max=50` 不变只延长训练（cosine 在 50ep 后
进入 plateau，等价于"用 50ep 的最终 lr 继续训练"），要么独立 grid search
`t_max` / `warmup_epochs` / `weight_decay` 等 schedule shape 参数。本实验
未做这一独立搜索（见 §12.2 与 §12.3）。
...
8. **朴素「more epoch = better」在所有三个模型上都不成立**：100 ep 扩展
   实验下，DN +0.062、ViT (new) +0.088、ViT (baseline) +2.515 MAE 全部
   退化，**50 ep 是三个模型共同的 budget sweet spot**。其中 ViT (baseline)
   退化最严重（test MAE +2.515，pred_std 11.54→8.51，Pearson r 0.696→0.429，
   部分回退向 collapse 状态）—— 把 `t_max` 与 `epochs` 绑定的同步延长方案
   在差配方上是**有害 budget**。详见 §11.8。
...
5. **ViT baseline 100 ep 已观察到部分回退**：本实验现在覆盖了 25 / 50 / 100 ep
   三个数据点（详见 §11.8），其中 100 ep 反而比 50 ep 退化（MAE 9.442 → 11.957，
   pred_std 11.54 → 8.51）。这说明 50 ep 已经接近 baseline 配方在
   `t_max=epochs` 同步延长方案下的最优，"再多 epoch 让 baseline 接近 new"
   的方向被证伪 —— 但**这并不排除独立调 schedule 之后 baseline 还能继续
   下降**，见下一条。
6. **未独立调 cosine `t_max` / warmup_epochs / weight_decay 等 schedule shape
   参数**：本实验 100 ep ablation 把 `t_max` 与 `epochs` 同步从 50 推到 100，
   即沿用「同步延长」原则，并未对 schedule shape 做独立 grid search。因此
   100 ep 配方**不是经过调优的最优 100 ep 配方**，所观察到的三个模型 100 ep
   都退化的现象，部分可归因于 schedule shape 与 budget 的耦合（即「拉长 cosine」
   而非「在 50ep schedule 之后继续训练」）。要把 budget 维度从 schedule shape
   维度中分离出来，需要补做相关网格搜索（见 §12.3）。
7. **2×2 ablation 路径不完整**：缺失「ViT (new) 25 epoch」这个 cell，
   因此无法在该 cell 上做 budget × recipe 的交互检验，只能在两条边路径
   上做近似可加性的描述性观察。
8. **没有做置信度估计 / 不确定性建模**：实验只输出 point estimate，
   未估计 per-sample 不确定性。

### 12.3 可扩展方向

1. **多 seed (≥3)**：取均值 + 标准差，给出 DN vs ViT (new) 差距的置信区间；
2. **ViT (new) 25 epoch**：补齐 budget × recipe 2×3 路径中仍缺失的 cell
   （现已覆盖 8/9，仅缺此一项），做正式交互检验；
3. **独立 grid search cosine `t_max` / `warmup_epochs` / `weight_decay`**：
   本实验 100 ep ablation 显示「`t_max` 同步到 epochs」是次优 schedule shape
   决策（见 §11.8 与 §12.2-6）。独立搜索这些参数可能让 ViT (baseline) 在
   100 ep 下接近甚至匹配 ViT (new) 50 ep 的水平 —— 一个直接可测的预测是：
   保持 `t_max=50` 不变但 `epochs=100`（即 50 ep cosine + 50 ep low-lr
   plateau），baseline 的 100 ep MAE 应当不再回退；
4. **更强数据增广（mixup / CutMix / RandAugment）**：看是否能进一步缩小
   长尾段误差；
5. **LLRD decay 网格搜索**（0.65 / 0.70 / 0.75 / 0.80 / 0.85），评估对
   ViT 训练稳定性的影响；
6. **Test-time augmentation**（horizontal flip 平均）：最便宜的预期提升手段；
7. **回归头换成 ordinal regression**（DEX / mean-variance / DLDL）：年龄
   估计常见技巧，可作进一步对比；
8. **更大数据集（IMDB-WIKI / UTKFace）重做对比**：检验 DN/ViT 打平结论
   在更大数据规模下是否仍成立。
...
**方法学发现来自 budget × recipe 2×3 ablation**：对 ViT 端，training budget
（25→50 ep）与 fine-tune recipe（baseline→new）两个维度各自贡献约 3.6-4.6 MAE
的独立增益，合起来把 ViT 从 25ep 塌缩状态（MAE 13.01）拉到与 DenseNet 相当的
水平（MAE 4.86）。但 budget 维度**非单调**：进一步推到 100 ep 后三个模型
全部退化（DN +0.06、ViT new +0.09、ViT baseline +2.51），其中 baseline
部分回退向 collapse（Pearson r 0.696→0.429）—— **50 ep 是 budget sweet
spot**，"更多 epoch = 更好"在所有三个模型上都被证伪，且 budget 与
schedule shape 不能孤立讨论。

---

附：所有实验输出文件位于 `results/{densenet, vit, vit_baseline, densenet_25ep,
vit_baseline_25ep, densenet_100ep, vit_100ep, vit_baseline_100ep}/` 与
`results/comparison/`；配置在
